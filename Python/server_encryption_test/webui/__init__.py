...
def hash_and_salt(password):
    ...
    password_hash.update(str(salt + request.form['password']).encode('UTF-8'))
...
def require_admin(func):
    ...
    def wrapper(*args, **kwargs):
        if 'username' in session and session['username'] == User.query.get(1).username:
    ...
    if not admin_user:
        if request.method == 'POST':
            if 'password' in request.form:
                ...
                db.session.commit()
                flash('Password set. Please login.')
                return redirect(url_for('webui.login'))
        return render_template('create_password.html')
    if request.method == 'POST': 
        if request.form['password']:
                password_hash = hashlib.sha256()
                password_hash.update(str(admin_user.salt + request.form['password']).encode('UTF-8'))
...
def change_password():
    if request.method == 'POST':
        if 'password' in request.form:
            ...
            db.session.commit()
            flash('Password reset successfully. Please log in again.')
            return redirect(url_for('webui.login'))
